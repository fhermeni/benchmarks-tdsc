...
case $TYPE in
	...
	for SET in nr li; do
	    ...
	    $EXE ${INPUT}/${SET} ${INPUT}/c0p5000  ${FOLDER}/entropy-rebuild.properties $o || exit 1
	done
	;;
    constraints)
	...
	for PCT in 0 33 66 100; do
		...
		$EXE ${INPUT}/${SET} ${INPUT}/c${PCT}p5000 $FOLDER/entropy-repair.properties $o || exit 1   
	    done
	done
	;;
    partitions)
	...
	for SIZE in 250 500 1000 2500; do  #5,000 has been made before
		...
		$EXE ${INPUT}/${SET} ${INPUT}/c100p${SIZE} $FOLDER/entropy-part.properties $o || exit 1	       
	    done
	done
	;;
