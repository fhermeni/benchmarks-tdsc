 ...
 */
public class FFDStyleVMAndHostAffinityLonelyRP extends CustomizablePlannerModule {
     ...
     */
    public FFDStyleVMAndHostAffinityLonelyRP(DurationEvaluator eval) {
        super(eval);
    }
    ...
    public TimedReconfigurationPlan compute(Configuration src,
        ...
        if (repair) {
            ...
            for (Node n : Configurations.futureOverloadedNodes(src)) {
                ...
                toRepair.addAll(src.getWaitings());
            }
        } else {
            toRepair = run.clone();
        }
        ...
        for (VirtualMachine vm : toRepair) {
            ...
            if (hasEnoughFreeResources(dst, current, vm) && !disallowBySpread(dst, current, vm) && !disallowByGather(dst, current, vm) && !disallowByWhiteList(dst, current, vm)) {
                ...
                for (Node n : dst.getOnlines()) {
                    if (n != src.getLocation(vm)) {
                        if (hasEnoughFreeResources(dst, n, vm) && !disallowBySpread(dst, current, vm) && !disallowByGather(dst, current, vm) && !disallowByWhiteList(dst, current, vm)) {
                            placed = true;
                            dst.setRunOn(vm, n);
        ...
        logger.info(getClass().getSimpleName() + ": " +nbViolations + "/" + cstrs.size() + " violation(s)");
